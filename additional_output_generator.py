...
def generate_additional_output(input_data: dict, result: dict, algorithm_name: str = "Solver"):
    ...
    _write_file("additional_output/10_kpi_summary.csv",
                _generate_kpi_csv(result))

    _write_file("additional_output/11_machine_metrics.csv",
                _generate_machine_csv(result))

    _write_file("additional_output/12_job_delivery.csv",
                _generate_delivery_csv(result))
...
def _generate_executive_dashboard_html(result: dict, input_data: dict, algorithm: str) -> str:
    ...
    avg_util = sum(m.get("utilization_percentage", 0) for m in mu.values()) / len(mu) if mu else 0

    horizon = input_data.get("planning_horizon", {}).get("end_time", 72)
    metadata = input_data.get("metadata", {})
    plant = metadata.get("plant_name", metadata.get("plant", "Plant"))

    elapsed = result.get("computation_metrics", {}).get("wall_time_s", 0)
...
def _generate_machine_utilization_html(result: dict, input_data: dict) -> str:
    ...
    machine_map = {m["machine_id"]: m.get("name", m["machine_id"]) for m in machines}

    rows = []
    sorted_mu = sorted(mu.items(), key=lambda x: x[1].get("utilization_percentage", 0), reverse=True)
    for mid, data in sorted_mu:
        util = data.get("utilization_percentage", 0)
        name = machine_map.get(mid, mid)
        proc = data.get("total_processing_hours", data.get("total_busy_time", 0))
        idle = data.get("idle_time_hours", data.get("idle_time", 0))
        njobs = data.get("num_jobs", data.get("num_jobs_processed", 0))
    ...
    avg_util = sum(d.get("utilization_percentage", 0) for d in mu.values()) / len(mu)

    body = f"""
    <div class="grid grid-3">
      {_kpi_card(f"{avg_util:.1f}%", "Average Utilization")}
      {_kpi_card(str(sum(1 for d in mu.values() if d.get("utilization_percentage",0) > 85)), "Bottleneck Machines")}
      {_kpi_card(str(sum(1 for d in mu.values() if d.get("utilization_percentage",0) < 50)), "Underutilized")}
    </div>
    <div class="card">
      <table>
...
def _generate_delivery_analysis_html(result: dict, input_data: dict) -> str:
    ...
    for j in jobs:
        ...
        grade = j.get("material_grade", "")
        prio = j.get("priority", "medium")
        due = m.get("due_date", j.get("due_date", horizon))
        comp = m.get("completion_time", 0)
        tard = m.get("tardiness", 0)
        on_time = m.get("on_time", False)
...
def _generate_machine_csv(result: dict) -> str:
    ...
    for mid, data in sorted(mu.items()):
        w.writerow([
            mid,
            f"{data.get('utilization_percentage', 0):.1f}",
            f"{data.get('total_processing_hours', data.get('total_busy_time', 0)):.1f}",
            f"{data.get('idle_time_hours', data.get('idle_time', 0)):.1f}",
            data.get("num_jobs", data.get("num_jobs_processed", 0)),
        ])
    return buf.getvalue()
